...
static t_pack_molecule* try_create_molecule(
...

    {
        ...
        if (molecule->num_blocks == 0) {
            failed = true;
            goto end_prolog;
        }

        if (list_of_pack_patterns[pack_pattern_index].root_block == nullptr) {
            failed = true;
            goto end_prolog;
        }
...
end_prolog:
    ...
    if (!failed && blk_id && try_expand_molecule(molecule, atom_molecules, blk_id, molecule->pack_pattern->root_block) == true) {
