...
static const t_pb_graph_edge* find_pb_graph_edge(const t_pb_graph_pin* driver, const t_pb_graph_pin* sink) {
    VTR_ASSERT(driver);
    VTR_ASSERT(sink);
...
static const t_pb_graph_edge* find_pb_graph_edge(IntraLbPbPinLookup& lb_pb_lookup, ClusterBlockId clb, int pb_route_idx) {
    auto& cluster_ctx = g_vpr_ctx.clustering();
    ...
    if(upstream_pb_route_idx >= 0) {
        const t_pb_graph_pin* pb_gpin = lb_pb_lookup.pb_gpin(type_index, pb_route_idx);
        const t_pb_graph_pin* upstream_pb_gpin = lb_pb_lookup.pb_gpin(type_index, upstream_pb_route_idx);
...
void ICE40HLCWriterVisitor::output_metadata(const t_pb *pb, t_metadata_dict* meta) {
    output_metadata(pb, t_offset(), meta);
}

void ICE40HLCWriterVisitor::output_metadata(const t_pb *pb, t_offset o, t_metadata_dict* meta) {
    if (meta != nullptr) {
        std::cout << " (metadata) ";
        if (meta->has(o, "hlc_cell")) {
            std::string cell_name = meta->get(o, "hlc_cell")->front().as_string();
            VTR_ASSERT(pb != nullptr);
            set_cell(cell_name, get_index(pb));
        }
        if (meta->has(o, "hlc_property")) {
            for (auto v : *(meta->get(o, "hlc_property"))) {
                if (current_cell_) {
...
void ICE40HLCWriterVisitor::visit_all_impl(const t_pb_route *top_pb_route, const t_pb* pb,
    ...
    output_metadata(pb, pb_type->meta);

    if (pb != nullptr) {
        if (pb->has_modes()) {
            auto mode = pb->get_mode();
            output_metadata(pb, mode->meta);
...
void ICE40HLCWriterVisitor::visit_clb_impl(ClusterBlockId blk_id, const t_pb* clb) {
    ...
    output_metadata(clb, pb_type->meta);

    std::cout << "CLB Interconnect info" << std::endl;
    auto type_index = cluster_ctx.clb_nlist.block_type(blk_id)->index;
    auto pb_graph_node = cluster_ctx.clb_nlist.block_pb(blk_id)->pb_graph_node;
    const t_pb_route* pb_routes = cluster_ctx.clb_nlist.block_pb(blk_id)->pb_route;

    for(int pin_idx = 0; pin_idx < pb_graph_node->total_pb_pins; pin_idx++) {
        AtomNetId atom_id = pb_routes[pin_idx].atom_net_id;

        std::cout << " Pin " << pin_idx << " ";

        const t_pb_graph_pin* pb_gpin = intra_lb_pb_pin_lookup_.pb_gpin(type_index, pin_idx);
        std::cout << vtr::string_fmt(
            "%s[%d].%s[%d]",
            pb_gpin->parent_node->pb_type->name,
            pb_gpin->parent_node->placement_index,
            pb_gpin->port->name,
            pb_gpin->pin_number);

        if (atom_id == AtomNetId::INVALID()) {
            std::cout << " - unused\n";
            continue;
        } else {
            std::cout << " - " << atom_ctx.nlist.net_name(atom_id);
        }

        int curr_pb_route_idx = pin_idx;
        while(pb_routes[curr_pb_route_idx].driver_pb_pin_id >= 0) {
            if (pb_routes[curr_pb_route_idx].driver_pb_pin_id == OPEN) {
                std::cout << " OPEN ";
                continue;
            }

            const t_pb_graph_edge* pb_edge = find_pb_graph_edge(intra_lb_pb_pin_lookup_, blk_id, curr_pb_route_idx);

            std::cout << " -> " << pb_edge->interconnect->name;
            output_metadata(nullptr, pb_edge->interconnect->meta);

            curr_pb_route_idx = pb_routes[curr_pb_route_idx].driver_pb_pin_id;
        }
        std::cout << "\n";
    }
}
